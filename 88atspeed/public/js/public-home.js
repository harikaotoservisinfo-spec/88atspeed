(function() {
    ...
    function getYildizColumnDef(kosular) {
        ...
        }, ...getYildizGostergeDepthColumnDefs(kosular)];
    }

    function gostergeDepthColumnLabel(k) {
        if (k === 1) return 'SON';
        return (k - 1) + ' ÖNCE';
    }

    function computeMaxGostergeK(kosular) {
        let max = 0;
        ...
        return Math.min(7, Math.max(1, max));
    }

    function getYildizGostergeDepthColumnDefs(kosular) {
        const maxK = computeMaxGostergeK(kosular);
        const defs = [];
        for (let k = 1; k <= maxK; k++) {
            defs.push({
                key: 'yildizGosK' + k,
                gostergeK: k,
                label: gostergeDepthColumnLabel(k),
                cls: 'pub-prog-yildiz-depth',
                colCls: 'pub-col-yildiz-depth pub-col-yildiz-depth-k' + k,
                title: 'GÖSTERGE · ' + (k === 1 ? 'son yarış' : (k - 1) + ' yarış önce')
            });
        }
        return defs;
    }
    ...
    function formatYildizGostergeDepthCell(h, k) {
        const list = (Array.isArray(h.yildizlar) ? h.yildizlar : [])
            .filter((y) => parseInt(y.k, 10) === k);
        if (!list.length) return '<span class="pub-prog-yildiz-empty">—</span>';
    ...
    function isYildizHtmlCol(key) {
        return (key && String(key).startsWith('yildizGosK')) || key === 'yildizTek' || key === 'yildizSon2Tek'
            || key === 'yildizSon1Tek' || key === 'yildizYuvarlak';
    }

    function computeYildizDepthColWidth(kosular, k) {
        let maxMarkers = 0;
        for (const race of kosular || []) {
            for (const h of race.horses || []) {
                const n = (h.yildizlar || []).filter((y) => parseInt(y.k, 10) === k).length;
    ...
    function applyYildizDepthColWidths(colWidths, kosular) {
        const maxK = computeMaxGostergeK(kosular);
        for (let k = 1; k <= maxK; k++) {
            colWidths['yildizGosK' + k] = computeYildizDepthColWidth(kosular, k);
        }
    }
    ...
    function programHorseCell(h, col, ctx) {
        ...
        if (col.gostergeK) return formatYildizGostergeDepthCell(h, col.gostergeK);
    ...
    function renderKayitEvalList(data) {
        ...
        applyYildizDepthColWidths(colWidths, kosular);
