...
const GosterimEngine = {
    COL: {
        ...
        TEST1_ENTEGRE: 18,
        TEST3_ENTEGRE: 19,
        TEST1: 20, TEST2: 21, TEST3: 22, TEST2_MINUS_TEST3: 23,
        TEST4: 24, TEST5: 25, TEST6: 26, TEST7: 27, TEST8: 28, TEST9: 29,
        FARK: 14, FARK8002: 17
    },
    ...
    _computeFark8002Sl(atKosu) {
        ...
        if (son800_2_sl === null || son800_1_sl === null) return null;
        return son800_2_sl - son800_1_sl;
    },

    _computeTest2MinusTest3Salise(atKosu, hedefMesafe) {
        const son800_1Salise = AtSpeedUtils.dereceToSalise(atKosu.son800_bir);
        const son800_1_sl = son800_1Salise ? son800_1Salise / 800 : null;
        ...
        const { test3_salise } = this._computeDrMetrics(atKosu, hedefMesafe);
        if (test2_salise === null || test3_salise === null) return null;
        return test2_salise - test3_salise;
    },
    ...
    collectMaviKenarTest9YakinAtlar(race, hedefMesafe, trends, enIyiler) {
        ...
        for (let j = 0; j < race.horses.length; j++) {
            ...
            if (!hasMaviKenar) continue;
            const test9 = trends.test7Farki[j];
            if (test9 === null || test9 === undefined) continue;
            adaylar.push({ j, abs: Math.abs(test9) });
        }
        const maviKenarTest9VurguAtlar = new Set();
        if (!adaylar.length) return { maviKenarTest9VurguAtlar };
        const minAbs = Math.min(...adaylar.map(a => a.abs));
        for (const { j, abs } of adaylar) {
            if (abs === minAbs) maviKenarTest9VurguAtlar.add(j);
        }
        return { maviKenarTest9VurguAtlar };
    },
    ...
    collectEnNegatifTest2MinusTest3(calcRace, hedefMesafe) {
        const candidates = [];
        for (let j = 0; j < calcRace.horses.length; j++) {
            const horse = calcRace.horses[j];
            const kosular = horse.kosular || [];
            for (let idx = 0; idx < kosular.length; idx++) {
                ...
                const { test3_salise } = this._computeDrMetrics(atKosu, hedefMesafe);
                if (test2_salise === null || test3_salise === null) continue;
                const fark = test2_salise - test3_salise;
                candidates.push({
                    j,
                    kosuKey: this._kosuKey(j, atKosu),
                    val: fark
                });
            }
        }
        const enNegatifTest2MinusTest3 = new Set();
        if (!candidates.length) return { enNegatifTest2MinusTest3 };
        let minVal = candidates[0].val;
        for (const c of candidates) {
            if (c.val < minVal) minVal = c.val;
        }
        if (minVal >= 0) return { enNegatifTest2MinusTest3 };
        for (const c of candidates) {
            if (c.val === minVal) enNegatifTest2MinusTest3.add(c.kosuKey);
        }
    ...
    /**
     * Görünüm SIRA=1: TEST1/TEST2/TEST3 arasında koşudaki en küçük (en iyi) 3 değer
     * → ilgili TEST hücresi fosforlu lacivert çerçeve vurgusu.
     */
    collectSiraBirTestEnKucukTop3(calcRace, hedefMesafe) {
        const pool = [];
        for (const { kosuKey, atKosu } of this._iterGosterimSiraBirKosular(calcRace)) {
            const { test1, test2, test3 } = this._computeTestSalise(atKosu, hedefMesafe);
            if (test1 !== null) pool.push({ kosuKey, col: 'TEST1', val: test1 });
            if (test2 !== null) pool.push({ kosuKey, col: 'TEST2', val: test2 });
            if (test3 !== null) pool.push({ kosuKey, col: 'TEST3', val: test3 });
        }
        pool.sort((a, b) => (a.val !== b.val ? a.val - b.val : a.kosuKey.localeCompare(b.kosuKey) || a.col.localeCompare(b.col)));
        const siraBirTestEnKucukTop3 = new Set();
        for (let i = 0; i < Math.min(3, pool.length); i++) {
            siraBirTestEnKucukTop3.add(pool[i].kosuKey + '|' + pool[i].col);
        }
        return { siraBirTestEnKucukTop3 };
    },
    ...
    collectSiraBirTop5T2m3Test9Yakin(calcRace, hedefMesafe, trends) {
        ...
        const siraBirTest9YanipSonen = new Set();
        if (!negatifler.length) return { siraBirTest9YanipSonen };
        negatifler.sort((a, b) => (a.val !== b.val ? a.val - b.val : a.j - b.j));
        const top5 = negatifler.slice(0, 5);
        const test9Aday = [];
        for (const item of top5) {
            const test9 = trends.test7Farki[item.j];
            if (test9 === null || test9 === undefined) continue;
            test9Aday.push({ j: item.j, abs: Math.abs(test9) });
        }
        test9Aday.sort((a, b) => (a.abs !== b.abs ? a.abs - b.abs : a.j - b.j));
        for (let i = 0; i < Math.min(3, test9Aday.length); i++) {
            siraBirTest9YanipSonen.add(test9Aday[i].j);
        }
        return { siraBirTest9YanipSonen };
    },
    ...
    buildRowValues(horse, atKosu, rowIndex, horseIndex, hedefMesafe, trends, enIyiler, hipodromSehir) {
        ...
        let test2_minus_test3_salise = null;
        if (test2_salise !== null && test3_salise !== null) {
            test2_minus_test3_salise = test2_salise - test3_salise;
        }
        ...
        const values = [
            ...
            AtSpeedUtils.saliseToFarkFormat(test2_minus_test3_salise),
    ...
    getCellClass(columnIndex, classes) {
        ...
        if (c === COL.TEST1) {
            ...
            if (classes.test9YanipClass) parts.push(classes.test9YanipClass);
            if (classes.test9Class) parts.push(classes.test9Class);
    ...
    buildRaceRows(race, options = {}) {
        ...
        Object.assign(enIyiler, this.collectSiraBirTestEnKucukTop3(calcRace, hedefMesafe));
