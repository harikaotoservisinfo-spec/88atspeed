...
const AtestSonGosterimCols = (function () {
    ...
    /** TEST1 + TEST2 + TEST3 hücrelerinin üçünün de lacivert-test-enkucuk-vurgu olması */
    function allTest123Kirmizi(gosRow) {
        ...
        for (let i = 0; i < cols.length; i++) {
            ...
            if (!cellClass || !/\blacivert-test-enkucuk-vurgu\b/.test(cellClass)) return false;
        }
        return true;
    }
    ...
    function isTest1Kirmizi(gosRow) {
        if (!gosRow?.classes || typeof GosterimEngine === 'undefined') return false;
        const cellClass = GosterimEngine.getCellClass(GosterimEngine.COL.TEST1, gosRow.classes);
        return !!(cellClass && /\blacivert-test-enkucuk-vurgu\b/.test(cellClass));
    }
    ...
    /** TEST1–TEST2 en yakın 3 at → sol SIRA (AT İSMİ öncesi) koyu mavi */
    function buildSiraMaviKeySet(race, meta, resolveKosular) {
        const keys = new Set();
        if (typeof GosterimEngine === 'undefined' || !race) return keys;
        const calcRace = enrichRace(race, resolveKosular);
        const hedefMesafe = parseInt(calcRace.mesafe, 10);
        const { test12YakinAtlar } = GosterimEngine.collectClosestTest12(calcRace, hedefMesafe);
        if (!test12YakinAtlar) return keys;
        for (const j of test12YakinAtlar) {
            const k = horseKey(calcRace.horses[j]);
            if (k) keys.add(k);
        }
        return keys;
    ...
    /**
     * TEST9 yanıp → +%45 · 8002-8001 sıfır (gri) → +%5 · TEST1/2/3 lacivert top-3 → +%25
        ...
        for (let i = 0; i < horseRows.length; i++) {
            ...
            if (maviCount === 4) {
                ...
                bonusTerms.push({
                    ...
                    points: SIRA_ATISMI_ATID_TARIH_MAVI_BONUS,
                    source: 'gosterim'
                });
            }
            ...
            if (gosRow) {
                ...
                if (flags.test123Kirmizi) {
                    ...
                    bonusTerms.push({
                        label: 'TEST1/2/3 lacivert (en küçük 3)',
