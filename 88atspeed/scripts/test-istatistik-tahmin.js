...
// smGec: mavi kenar +1 = 10×60/1000 puan
...
if (sl802T.score !== 1) {
    console.error('FAIL: sl802 maviKenar +1 = 10×110/1000 puan', sl802T.score);
    process.exit(1);
}
...
const oldMaviPts = oldMaviTerm?.points || 0;
if (!sonMaviTerm) {
    console.error('FAIL: test1 maviKenar SON terimi', sonMaviTerm, oldMaviTerm);
    process.exit(1);
}
if (sonMaviTerm.points <= oldMaviPts) {
    console.error('FAIL: SON maviKenar > 6 ÖNCE maviKenar', sonMaviTerm.points, oldMaviPts);
    process.exit(1);
}
// SON: 1×10×1×250/1000=3; 6 ÖNCE: 1×10×(1/7)×250/1000≈0
...
TE.setSelectedMetric('son8001');
TE.setCalcMode(TE.CALC_MODE_SOLO);
const zeroSonT = TE.computeRowTahmin(
    { son8001Depths: [{ pct: 0 }, { pct: 80 }] },
    [],
    null,
    { maxDepth1: 7 }
);
