const puppeteer = require('puppeteer');

(async () => {
    const browser = await puppeteer.launch({
        headless: true,
        executablePath: '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome'
    });
    const page = await browser.newPage();
    
    const sonuclar = [];
    
    for (let kosuNo = 1; kosuNo <= 8; kosuNo++) {
        const url = `https://www.tjk.org/TR/YarisSever/Info/Page/GunlukYarisProgrami?QueryParameter_Tarih=07/05/2026&SehirId=5&SehirAdi=Ankara&RaceNo=${kosuNo}&Era=today`;
        
        console.log(`📄 ${kosuNo}. Koşu sayfasına gidiliyor...`);
        await page.goto(url, { waitUntil: 'networkidle2', timeout: 60000 });
        
        // AJAX ile yüklenen içeriği bekle
        await page.waitForSelector('.race-info', { timeout: 10000 });
        await new Promise(r => setTimeout(r, 3000));
        
        const bilgi = await page.evaluate((kosuNo) => {
            // Sayfanın tam metnini al (AJAX sonrası)
            const bodyText = document.body.innerText;
            
            // Koşu başlığını ve mesafeyi bul
            const lines = bodyText.split('\n');
            let mesafe = '?';
            let kosuBasligi = '';
            
            for (let i = 0; i < lines.length; i++) {
                const line = lines[i].trim();
                // Koşu başlığı pattern'i: "1. Koşu 17.45"
                if (line.match(/^\d+\.\s*Koşu\s+\d+\.\d+/)) {
                    kosuBasligi = line;
                    // Bir sonraki satırda mesafe var
                    if (i + 1 < lines.length) {
                        const nextLine = lines[i + 1];
                        // Mesafe pattern'i
                        const match = nextLine.match(/(\d{3,4})\s*(Kum|Çim)/);
                        if (match) {
                            mesafe = match[1];
                            break;
                        }
                    }
                }
            }
            
            // Eğer bulunamazsa farklı bir yöntem dene
            if (mesafe === '?') {
                const match = bodyText.match(/\d+\.\s*Koşu[\s\S]*?(\d{3,4})\s*(Kum|Çim)/);
                if (match) mesafe = match[1];
            }
            
            return { kosuNo: kosuNo, mesafe: mesafe, baslik: kosuBasligi };
        }, kosuNo);
        
        sonuclar.push(bilgi);
        console.log(`   ✅ ${bilgi.baslik || bilgi.kosuNo + '. Koşu'} -> Mesafe: ${bilgi.mesafe} metre`);
    }
    
    console.log('\n📊 TÜM KOŞU MESAFELERİ:');
    console.log(JSON.stringify(sonuclar, null, 2));
    
    await browser.close();
})();
